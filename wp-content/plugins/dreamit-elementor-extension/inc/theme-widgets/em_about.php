...
add_action('widgets_init', 'consen_about_us_widgets');

if(!function_exists('consen_about_us_widgets')){
	
	function consen_about_us_widgets(){
		
		register_widget('consen_about_us_Widget');
		
	}
}

if(!class_exists('consen_about_us_Widget')){
	
	class consen_about_us_Widget extends WP_Widget {
						...
						<div class="footer-address">
							...
							<div class="footer_s_inner"> 
								...
								<div class="footer-sociala-info">				
									<p><?php echo esc_html($instance['address1']); ?></p>
								</div> 
							</div> 
							<?php endif; ?>
							...
							<div class="footer_s_inner"> 
								...
								<div class="footer-sociala-info">   
										<p><?php echo esc_html($instance['telephone']); ?></p>
								</div>
							</div>
							<?php endif; ?>
							<?php if(!empty($instance['email']) && isset($instance['email'])): ?>						
								<div class="footer_s_inner"> 
									...
									<div class="footer-sociala-info">  
										<p><?php echo esc_html($instance['email']); ?></p>									
									</div>
								</div>
							<?php endif; ?>
						</div>
		...
		public function form($instance){
			...
			<p>
				<label style="display:block;" for="<?php echo esc_attr($this->get_field_id('image_uri')); ?>"><?php esc_html_e('Upload Image:','consen');?></label>
				
				<img class="custom_media_image" src="<?php if(!empty($instance['image_uri'])){echo esc_url($instance['image_uri']);} ?>" style="margin:0;padding:0;max-width:100px;display:inline-block" />
				...
				<a href="#" id="custom_media_button" style="margin-top:10px;" class="button button-primary custom_media_button"><?php esc_html_e('Upload', 'consen'); ?></a>
			</p>

			<p>

				<label for="<?php echo esc_attr($this->get_field_id('content')); ?>"><?php esc_html_e('About Text:','consen');?></label>
				<textarea class="widefat" rows="5" type="text" id="<?php echo esc_attr($this->get_field_id('content')); ?>" name="<?php echo esc_attr($this->get_field_name('content')); ?>" ><?php echo esc_html($instance['content']); ?></textarea>
				<span> <b><?php esc_html_e('Note:','consen');?></b><?php esc_html_e('use','consen');?> <b> &ltbr&gt </b><?php esc_html_e(' for line break.','consen');?></span>

			</p>
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('address1')); ?>"><?php esc_html_e('Address:','consen');?></label>
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('telephone')); ?>"><?php esc_html_e('Telephone:','consen');?></label>
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('email')); ?>"><?php esc_html_e('Email & Web:','consen');?></label>
